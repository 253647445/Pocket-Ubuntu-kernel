...
static int bq24190_read(struct bq24190_dev_info *bdi, u8 reg, u8 *data)
{
	...
	if (ret < 0){
printk("error read bq24190\n");
disable_irq(182);
udelay(50);
enable_irq(182);

		return ret;}
//printk("bq24190_read reg====%d,ret===%d\n");
...
{
	printk("bq24190_write reg===%d,data====%d\n",reg,data);
...
{
	u8 idx;
printk("bq24190_set_field_val====reg====%d\n",reg);
		...
		union power_supply_propval *val)
{
	u8 v;
	int type, ret;
printk("bq2410_charge_get_charge_type\n");
	ret = bq24190_read_mask(bdi, BQ24190_REG_POC,
		...
		const union power_supply_propval *val)
{
	 ...
	 */
printk("bq24190_charger_set_charge_type\n");
	switch (val->intval) {
		...
		union power_supply_propval *val)
{
	u8 v;
	int curr, ret;
printk("bq24190_charger_get_current\n");
...
printk("bq24190_charger_set_current\n");
	ret = bq24190_read_mask(bdi, BQ24190_REG_CCC,
			BQ24190_REG_CCC_FORCE_20PCT_MASK,
			BQ24190_REG_CCC_FORCE_20PCT_SHIFT, &v);
	if (ret < 0)
		...
		const union power_supply_propval *val)
{
	...
	dev_dbg(bdi->dev, "prop: %d\n", psp);
printk("bq24190_charger_set_property =====%d\n",psp);
...
static irqreturn_t bq24190_irq_handler_thread(int irq, void *data)
{
	...
	u8 ss_reg = 0, f_reg = 0;
	int ret;
printk("bq24190_irq_handler_thread\n");

	pm_runtime_get_sync(bdi->dev);
printk("pm_runtime_get_sync\n");
	ret = bq24190_write_mask(bdi,BQ24190_REG_ISC,BQ24190_REG_ISC_IINLIM_MASK,
					BQ24190_REG_ISC_IINLIM_SHIFT,
					7);
u8 v;
bq24190_read(bdi,BQ24190_REG_ISC,&v);
printk("ret lim=====%d\n",v);
	ret = bq24190_read(bdi, BQ24190_REG_SS, &ss_reg);
	if (ret < 0) {
...
printk("bq24190_hw_init\n");
	...
	if (ret < 0)
		goto out;
printk("vvvvvv==========%d,bdi->model======%d\n",v,bdi->model);
	if (v != BQ24190_REG_VPRS_PN_24192I) {
		ret = -ENODEV;
		goto out;
	}
...
printk("bq24190_setup_pdatafggfg\n");
...
//chip
	...
	{
	     gpio=devm_gpiod_get_index(dev,"bq24190_int",0,GPIOD_IN);
	     if(IS_ERR(gpio)){
		dev_err(dev,"acpi get index failed\n");
		ret=PTR_ERR(gpio);
		goto out1;	
	      }else {
		ret = gpiod_direction_input(gpio);
		if(ret<0)
		dev_err(dev,"chrg set irq erro\n");
      		bdi->irq=gpiod_to_irq(gpio);
		}
	}
if(bdi->irq<0)
bdi->irq=client->irq;
...
ret=bq24190_write_mask(bdi, BQ24190_REG_CCC,
			BQ24190_REG_CCC_ICHG_MASK, BQ24190_REG_CCC_ICHG_SHIFT,32);
bq24190_read(bdi,BQ24190_REG_CCC,&v);
printk("ret cur=====%d\n",v);
ret=bq24190_write_mask(bdi, BQ24190_REG_CVC,
			BQ24190_REG_CVC_VREG_MASK, BQ24190_REG_CVC_VREG_SHIFT,54);
/*
...
{"bq24192",},
{"bxt_wcove_ext_charger",BQ24190_REG_VPRS_PN_24190},
{"cht_wcove_ext_chgr",},
//{ "TXN24292:00", BQ24190_REG_VPRS_PN_24190 },
	{ },
};
