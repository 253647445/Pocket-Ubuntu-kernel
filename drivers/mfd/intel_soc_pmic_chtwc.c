...
printk("cht_wc_byte_reg_read\n");
	client->addr = (reg & REG_ADDR_MASK) >> REG_ADDR_SHIFT;
	ret = i2c_smbus_read_byte_data(client, reg & REG_OFFSET_MASK);
	client->addr = orig_addr;

	if (ret < 0)
{      printk("i2c_smbus_read_byte_data err\n");
		return ret;
}
			...
			const struct i2c_device_id *i2c_id)
{
	...
	int ret;
printk("cht_ddddddddddddddddddddddddddddddddddddddd\n");
	...
	if (IS_ERR(pmic->regmap))
		{
printk("devm_regmap init \n");
return PTR_ERR(pmic->regmap);}
printk("devm_regmap_irq_chip\n");
...
printk("cht_wc_suspend pmic->irq===%d\n",pmic->irq);
	disable_irq(pmic->irq);
	return 0;
}
